...
public class WarriorAI : Unit
{
    ...
    // ── Defend mode (allied troops from buildings) ───────────────────────────
    ...
    void UpdateDefend()
    {
        ...
        _lockedTarget = null;   // target died or was null

        // ── Scan for a new target ────────────────────────────────────────────
        Unit enemy = FindNearestEnemy(_patrolRadius);
        if (enemy != null)
        {
            _lockedTarget = enemy;   // lock on — will pursue until dead
            float dist = Vector3.Distance(transform.position, enemy.transform.position);
            if (dist > attackRange)
            {
                MoveToward(enemy.transform.position);
    ...
    void AttackUnit(Unit target)
    {
        ...
        else if (Time.time >= _attackAnimEndTime)
        {
